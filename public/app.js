...
async function api(path, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(
      (data && data.error) || "Erreur serveur."
    );
  }
...
function switchView(view) {
  ...
  if (view === "photos")
    loadFilteredFast("image", "photo-grid", renderPhotoGrid);

  if (view === "videos")
    loadFilteredFast("video", "video-grid", (g, files) => renderFileGrid(g, files));

  if (view === "music")
    loadFilteredFast("audio", "music-grid", (g, files) => renderFileGrid(g, files));

  if (view === "docs")
    loadFilteredFast("doc", "docs-grid", (g, files) => renderFileGrid(g, files));
...
function filterFiles(query) {
  if (!query) {
    renderFileGrid($id("file-grid"), state.allFiles);
    return;
  }

  const filtered = state.allFiles.filter(
...
async function loadFiles() {
  ...
  try {
    ...
    grid.innerHTML =
      `<div class="empty-state"><img src="assets/icons/crystal/file-important.png" alt=""/><p>${err.message}</p></div>`;
  }
}
...
function updateFileSectionTitle() {
  ...
  el.innerHTML =
    `<img src="assets/icons/crystal/folder.png" alt="" />${escapeHtml(name)}`;
}
...
function renderFileGrid(grid, files) {
  ...
  if (!files || files.length === 0) {
    grid.innerHTML =
      `<div class="empty-state"><img src="assets/icons/crystal/folder.png" alt=""/><p>Aucun fichier ici.</p></div>`;
    return;
  }
...
function buildFileItem(f, isGrid) {
  ...
  if (!f.is_folder) {
    ...
    dlBtn.innerHTML =
      `<img src="assets/icons/crystal/action-down.png" alt="" />`;
  ...
  delBtn.innerHTML =
    `<img src="assets/icons/crystal/action-delete.png" alt="" />`;
...
function renderPhotoGrid(grid, files) {
  ...
  if (!files || files.length === 0) {
    grid.innerHTML =
      `<div class="empty-state"><img src="assets/icons/crystal/folder-image.png" alt=""/><p>Aucune photo.</p></div>`;
    return;
  }
...
if (dropzone) {
  ...
  dropzone.addEventListener(
    "drop",
    e => uploadFiles(e.dataTransfer.files)
  );
}
...
async function uploadFiles(fileList) {
  ...
  showToast(
    `Envoi de ${files.length} fichier${files.length > 1 ? "s" : ""}…`
  );
...
$id("new-folder-btn").addEventListener(
  "click",
  () => {
    sfx("balloon");
    ...
    setTimeout(
      () => $id("folder-name-input").focus(),
      80
    );
  }
);

$id("folder-modal-cancel").addEventListener(
  "click",
  () => {
    sfx("click");
...
$id("folder-modal").addEventListener(
  "click",
  e => {
...
$id("folder-modal-confirm").addEventListener(
  "click",
  async () => {
    const name =
      $id("folder-name-input").value.trim();
...
$id("folder-name-input").addEventListener(
  "keydown",
  e => {
    if (e.key === "Enter")
...
$id("lightbox-close").addEventListener(
  "click",
  () => hide($id("lightbox"))
);

$id("lightbox").addEventListener(
  "click",
  e => {
...
function openContextMenu(e, f) {
  ...
  menu.style.left =
    `${Math.min(e.clientX, window.innerWidth - 180)}px`;

  menu.style.top =
    `${Math.min(e.clientY, window.innerHeight - 120)}px`;
...
$id("ctx-download").addEventListener(
  "click",
  () => {
...
$id("ctx-rename").addEventListener(
  "click",
  async () => {
    const f = state.contextTarget;
...
$id("ctx-delete").addEventListener(
  "click",
  async () => {
    const f = state.contextTarget;
...
$id("new-note-btn").addEventListener(
  "click",
  async () => {
    sfx("balloon");
...
$id("delete-note-btn").addEventListener(
  "click",
  async () => {
    if (
      !state.activeNoteId ||
      !confirm("Supprimer cette note ?")
    )
      return;
...
$id("save-note-btn").addEventListener(
  "click",
  async () => {
...
async function saveActiveNote() {
  ...
  $id("note-status").textContent =
    `Sauvegardé à ${new Date().toLocaleTimeString("fr-FR", {
      hour: "2-digit",
      minute: "2-digit"
    })}`;
...
if (musicBtn) {
  musicBtn.addEventListener(
    ...
    e => {
      ...
      const isExpanded =
        musicBubble.classList.contains(
          "expanded"
        );
